...
class QrScannerActivity : AppCompatActivity() {
    ...
    private val scaleGestureDetector by lazy {
        ScaleGestureDetector(this, object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
            override fun onScale(detector: ScaleGestureDetector): Boolean {
                if (maxDigitalZoom <= 1f) return false

                digitalZoom = (digitalZoom * detector.scaleFactor)
                    .coerceIn(1f, maxDigitalZoom)
                updatePreviewZoom()
                return true
            }
        })
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.qrScannerZoomOut.setOnClickListener { changeZoom(1f / 1.25f) }
        binding.qrScannerZoomIn.setOnClickListener { changeZoom(1.25f) }
        updateZoomControls()
        binding.qrScannerPreview.surfaceTextureListener = textureListener
        binding.qrScannerPreview.setOnTouchListener { _, event ->
    ...
    private fun createCaptureSession() {
        ...
        val requestBuilder = createPreviewRequest(device, previewSurface, analysisSurface)
    ...
    private fun setZoom(requestBuilder: CaptureRequest.Builder) {
        val activeRect = sensorRect ?: return
        if (digitalZoom <= 1f) {
            requestBuilder.set(CaptureRequest.SCALER_CROP_REGION, activeRect)
            return
        }

        val cropWidth = (activeRect.width() / digitalZoom).toInt()
        val cropHeight = (activeRect.height() / digitalZoom).toInt()
        val left = activeRect.centerX() - cropWidth / 2
        val top = activeRect.centerY() - cropHeight / 2
        requestBuilder.set(
    ...
    private fun updatePreviewZoom() {
        ...
        runCatching {
            session.setRepeatingRequest(
                createPreviewRequest(device, previewSurface, analysisSurface).build(),
                null,
                backgroundHandler,
            )
        }
    ...
    private fun changeZoom(multiplier: Float) {
        if (maxDigitalZoom <= 1f) return
        digitalZoom = (digitalZoom * multiplier).coerceIn(1f, maxDigitalZoom)
        updatePreviewZoom()
    }

    private fun updateZoomControls() {
        if (!::binding.isInitialized) return
        binding.qrScannerZoomOut.isEnabled = digitalZoom > 1f
        binding.qrScannerZoomIn.isEnabled = digitalZoom < maxDigitalZoom
    }
