...
class MainViewModel : ViewModel() {
    ...
    fun checkUpdate() = viewModelScope.launch(Dispatchers.IO) {
        ...
        try {
            ...
            val compatibleReleases = newReleases.mapNotNull { release ->
                InAppUpdater.selectAsset(release, BuildConfig.APP_LAYOUT)?.let { asset ->
                    release to asset
                }
            }
            if (compatibleReleases.isEmpty()) return@launch

            _state.emit(
                State.SuccessCheckingUpdate(
                    newReleases = compatibleReleases.map { it.first },
                    asset = compatibleReleases.first().second,
                ),
            )
        } catch (e: Exception) {
