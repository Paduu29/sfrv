...
interface TvShowDao {
    ...
    @Query("UPDATE tv_shows SET rating = :rating WHERE id = :id")
    suspend fun updateRating(id: String, rating: Double?)

    @Query("UPDATE tv_shows SET rating = :rating, released = COALESCE(:released, released) WHERE id = :id")
    suspend fun updateRatingAndReleased(id: String, rating: Double?, released: String?)

    @Query("UPDATE tv_shows SET imdbId = :imdbId WHERE id = :id")
    fun updateImdbId(id: String, imdbId: String)
    ...
    @Transaction
    fun restoreFromBackup(tvShow: TvShow) {
        val existing = getById(tvShow.id)
        if (existing == null) {
            insert(tvShow)
            return
        ...
        if (existing.title.isBlank()) existing.title = tvShow.title
        if (existing.poster.isNullOrBlank()) existing.poster = tvShow.poster
        if (existing.banner.isNullOrBlank()) existing.banner = tvShow.banner
        if (existing.imdbId.isNullOrBlank()) existing.imdbId = tvShow.imdbId
        if (tvShow.backupStateTimestamp() > existing.backupStateTimestamp()) {
            existing.isFavorite = tvShow.isFavorite
            existing.favoritedAtMillis = tvShow.favoritedAtMillis
            existing.isWatching = tvShow.isWatching
            existing.lastPlayedAtMillis = tvShow.lastPlayedAtMillis
            existing.lastPlayedEpisodeId = tvShow.lastPlayedEpisodeId
        }
        update(existing)
    }
