...
open class VidplayExtractor : Extractor() {
    ...
    override val aliasUrls = listOf("https://vidplays.fun")
    open val key = "https://raw.githubusercontent.com/Ciarands/vidsrc-keys/main/keys.json"

    override suspend fun extract(link: String): Video {
        if (link.substringAfter("://").substringBefore('/').equals("vidplays.fun", true)) {
            return BrowserEmbedExtractor(
                name = "Vidplays",
                mainUrl = "https://vidplays.fun",
            ).extract(link)
        }
    ...
    data class Keys(
        ...
        class Deserializer : JsonDeserializer<Keys> {
            override fun deserialize(
                json: JsonElement?,
                typeOfT: Type?,
                context: JsonDeserializationContext?,
                ...
                if (json?.isJsonObject == true) {
                    return Gson().fromJson(json, Keys::class.java)
                }

                val values = json?.takeIf(JsonElement::isJsonArray)
                    ?.asJsonArray?.map { it.asString }
                    .orEmpty()
                if (values.size == 3) {
                    return Keys(
                        encrypt = listOf("", values[0], values[1]),
                        decrypt = listOf("", values[2]),
                    )
                }
                throw IllegalStateException("Unsupported legacy Vidplay key shape")
            }
        }
    }
}
