...
open class StreamWishExtractor : Extractor() {
    ...
    override suspend fun extract(link: String): Video {
        ...
        val requestReferer = referer.ifBlank {
        ...
        // Do not leak one host's referer into the next extraction performed by
        // this registry singleton. Explicit referers are consumed once.
        referer = ""
        val service = Service.build(mainUrl)
        val redirectedUrl = resolveRedirectWithWebView(context, link, mainUrl)
        val document = service.get(redirectedUrl, referer = requestReferer)
