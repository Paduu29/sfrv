...
import com.streamflixrevanced.streamflix.utils.topRoundedPoster
...
class TvShowViewHolder(
    ...
    private fun ratingLabel(rating: Double?): String =
        rating?.takeIf { it > 0.0 }?.let { String.format(Locale.ROOT, "%.1f", it) }
            ?: if (UserPreferences.enableTmdb) "N/A" else ""

    private fun hasRatingBadge(rating: Double?): Boolean =
        UserPreferences.enableTmdb || (rating != null && rating > 0.0)
    ...
    private fun isIptvProvider(): Boolean {
        ...
        return provider is IptvProvider &&
            !tvShow.id.startsWith("movie.") &&
            !tvShow.id.startsWith("series.")
    }

    private fun episodeBadgeText(): String {
        if (isIptvProvider()) return ""
        return tvShow.lastPlayedEpisode?.let { episode ->
    ...
    private fun handleDirectPlay(navController: NavController) {
        val liveTitle = tvShow.currentProgram ?: tvShow.title
        val videoType = Video.Type.Episode(
            ...
            tvShow = Video.Type.Episode.TvShow(
                ...
                currentProgram = tvShow.currentProgram,
            ),
    ...
    private fun resolveEpisodeSeason(episode: Episode?): Season? {
        ...
        // Prefer the season that actually contains this episode. The Episode may
        // carry a stale/partial Season object from Room with the wrong number.
        return tvShow.seasons.firstOrNull { season ->
            season.episodes.any { it.id == episode.id }
        } ?: tvShow.seasons.firstOrNull { season ->
            season.id == seasonKey ||
                season.id == currentSeason?.id
        } ?: currentSeason?.takeIf { it.number != 0 }
            ?: tvShow.seasons.firstOrNull { season ->
                episode.number != 0 && season.episodes.any {
                    it.number == episode.number && it.title == episode.title
                }
            }
    }

    private fun resolvedSeasonNumber(season: Season?): Int {
        if (season == null) return 1

        val titleNumber = Regex("\\bSeason\\s+(\\d+)\\b", RegexOption.IGNORE_CASE)
            .find(season.title.orEmpty())
            ?.groupValues
            ?.getOrNull(1)
            ?.toIntOrNull()

        return titleNumber ?: season.number.takeIf { it != 0 } ?: 1
    }

    private fun setPoster(imageView: ImageView, roundTop: Boolean = false) {
    ...
    private fun displayMobileItem(binding: ItemTvShowMobileBinding) {
        ...
        binding.tvTvShowYearBadge.apply {
            text = tvShow.released?.format("yyyy") ?: ""
            visibility = if (text.isNullOrEmpty()) View.GONE else View.VISIBLE
        }
        binding.tvTvShowTitle.text = tvShow.title

        binding.llTvShowRatingBadge.visibility = if (hasRatingBadge(tvShow.rating)) View.VISIBLE else View.GONE
        binding.tvTvShowRating.text = ratingLabel(tvShow.rating)
    }
    ...
    private fun displayTvItem(binding: ItemTvShowTvBinding) {
        ...
        setPoster(binding.ivTvShowPoster, roundTop = true)
        bindRibbons(binding.ivTvShowFavoriteRibbon, binding.ivTvShowWatchedRibbon)
        binding.tvTvShowQuality.apply {
            text = tvShow.quality ?: ""
            isVisible = !text.isNullOrEmpty()
    ...
    private fun displayTvShowMobile(binding: ContentTvShowMobileBinding) {
        ...
        binding.btnTvShowWatchNow.apply {
            ...
            setOnClickListener {
                if (isIptvProvider()) {
                    ...
                    val videoType = Video.Type.Episode(
                        ...
                        season = Video.Type.Episode.Season(
                            number = resolvedSeasonNumber(episodeSeason),
            ...
            text = if (isIptvProvider()) context.getString(R.string.movie_watch_now) else context.getString(R.string.tv_show_watch_season_episode, resolvedSeasonNumber(episodeSeason), episodeToWatch?.number ?: 1)
        }
                ...
                checkProviderAndRun {
                    itemView.findViewTreeLifecycleOwner()?.lifecycleScope?.launch(Dispatchers.IO) {
                        ...
                        UserPreferences.currentProvider?.let { provider ->
                            dao.getById(resolvedTvShow.id)?.let { persisted ->
                                UserDataCache.syncTvShowToCache(context, provider, persisted)
                            }
                        }
    ...
    private fun displaySeasonsMobile(binding: ContentTvShowSeasonsMobileBinding) {
        binding.rvTvShowSeasons.apply {
            val seasonAdapter = (adapter as? AppAdapter) ?: AppAdapter().also { adapter = it }
            seasonAdapter.submitList(tvShow.seasons.onEach { it.itemType = AppAdapter.Type.SEASON_MOBILE_ITEM })
    ...
    private fun displaySeasonsTv(binding: ContentTvShowSeasonsTvBinding) {
        binding.hgvTvShowSeasons.apply {
            ...
            val seasonAdapter = (adapter as? AppAdapter) ?: AppAdapter().also { adapter = it }
            seasonAdapter.submitList(tvShow.seasons.onEach { it.itemType = AppAdapter.Type.SEASON_TV_ITEM })
            setItemSpacing(20)
        }
    }
