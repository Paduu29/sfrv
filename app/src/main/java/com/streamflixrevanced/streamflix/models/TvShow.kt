...
class TvShow(
    ...
    val episodeToWatch: Episode?
        get() {
            ...
            val latestHistory = episodes
                .withIndex()
                .filter { it.value.watchHistory != null }
                .maxByOrNull {
                    it.value.watchHistory?.lastEngagementTimeUtcMillis ?: Long.MIN_VALUE
                }
            val episode = when {
                // A resume point on a later episode is more useful than the next
                // episode after the last completed one.
                latestHistory != null && latestHistory.index > lastWatchedIndex ->
                    latestHistory.value
                lastWatchedIndex >= 0 ->
                    episodes.getOrNull(lastWatchedIndex + 1)
                latestHistory != null -> latestHistory.value
                else -> null
            }
    ...
    fun copy(
        ...
        currentProgram = this@TvShow.currentProgram
    }
