...
import com.streamflixrevanced.streamflix.databinding.FragmentFavoritesMobileBinding
...
class FavoritesMobileFragment : Fragment() {
    ...
    private fun itemKey(item: AppAdapter.Item): String? = when (item) {
        ...
        is LiveChannel -> selectionKey(FavoritesViewModel.Section.TV_SHOWS, item.id)
        else -> null
    }
            ...
            listOf<AppAdapter.Item>(FavoriteSectionHeader(title, favoriteSection.section)) +
                favoriteSection.items.onEach { item ->
                    item.itemType = when (item) {
                        ...
                        is LiveChannel -> AppAdapter.Type.LIVE_CHANNEL_GRID_MOBILE_ITEM
                        else -> item.itemType
                    }
                }
        }
