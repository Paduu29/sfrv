...
import com.streamflixrevanced.streamflix.utils.ProfileAvatarPicker
import com.streamflixrevanced.streamflix.utils.ProfileSwitchPinGuard
...
class SettingsTvFragment : LeanbackPreferenceFragmentCompat() {
    ...
    private val screenScrollStates = mutableMapOf<String?, Parcelable>()
    ...
    override fun onPreferenceTreeClick(preference: Preference): Boolean {
        if (preference is PreferenceScreen && !preference.key.isNullOrBlank()) {
            listView?.layoutManager?.onSaveInstanceState()?.let {
                screenScrollStates[currentScreenState.rootKey] = it
            }
    ...
    private fun renderCurrentScreen() {
        ...
        installPlaceholderIcons(preferenceScreen)
        if (::backupRestoreManager.isInitialized) {
            displaySettings()
        }
        applyScreenTitle()
        view?.post {
            screenScrollStates[currentScreenState.rootKey]?.let { state ->
    ...
    private fun installPlaceholderIcons(group: PreferenceGroup) {
        for (index in 0 until group.preferenceCount) {
            val preference = group.getPreference(index)
            if (preference.icon == null) {
                preference.icon = requireContext().getDrawable(R.drawable.ic_settings_placeholder)
            }
            if (preference is PreferenceGroup) {
                installPlaceholderIcons(preference)
            }
        }
    }
    ...
    private fun displaySettings() {
        CloudAccountSettingsController.bind(this, lifecycleScope) { key ->
        ...
        findPreference<Preference>("p_settings_help")?.setOnPreferenceClickListener {
            startActivity(
                Intent(
                    Intent.ACTION_VIEW,
                    Uri.parse("https://github.com/PaduU29-SFRV/streamflix-revanced/issues")
                )
            )
            true
        }
        ...
        findPreference<SwitchPreference>("AUTO_SKIP_INTRO")?.apply {
            isChecked = UserPreferences.autoSkipIntro
            setOnPreferenceChangeListener { _, newValue ->
                UserPreferences.autoSkipIntro = newValue as Boolean
                true
            }
        }
        ...
        findPreference<MultiSelectListPreference>("SUBTITLE_LANGUAGES")?.apply {
            entries = entryValues.map { value ->
                val languageCode = value.toString()
                if (languageCode == "all") {
            ...
            values = UserPreferences.subtitleLanguages
            summaryProvider = Preference.SummaryProvider<MultiSelectListPreference> { preference ->
                when {
                    "all" in preference.values ->
                        getString(R.string.settings_subtitle_languages_all)
                    preference.values.isEmpty() ->
                        getString(R.string.settings_subtitle_languages_none)
                    else -> preference.entryValues
                        .mapIndexedNotNull { index, value ->
                            preference.entries.getOrNull(index)
                                ?.takeIf { value.toString() in preference.values }
                        }
                        .joinToString()
                }
            }
            setOnPreferenceChangeListener { _, newValue ->
                val requested = (newValue as? Set<*>)
                    ?.filterIsInstance<String>()
                    ?.toSet()
                    .orEmpty()
                val normalized = when {
                    "all" in requested && "all" !in values -> setOf("all")
                    "all" in requested -> requested - "all"
                    else -> requested
                }
                values = normalized
        ...
        findPreference<Preference>("key_profile_edit")?.setOnPreferenceClickListener {
            ...
            if (active != null) showProfileActionsDialogTv(active)
            true
        }
    ...
    private fun updateOverviewLabels() {
        ...
        findPreference<PreferenceScreen>("screen_cloud_sync")?.summary =
            if (SupabaseSettings.isConfigured) {
                getString(R.string.cloud_sync_screen_summary)
            } else {
                getString(R.string.supabase_config_setup_required)
            }
    ...
    private fun updateProviderVisibilityState() {
        ...
        val hasConfigProvider = UserPreferences.currentProvider is ProviderConfigUrl
        val hasSpecificOptions = isStreamingCommunity || isCuevana || isPoseidon || isAnimeOnlineNinja || isHdFull
    ...
    private fun bindHdFullPreferredServer() {
        val preference = findPreference<ListPreference>("provider_hdfull_preferred_server") ?: return
        val currentValue = UserPreferences.getProviderCache(
            HdFullProvider,
            UserPreferences.PROVIDER_PREFERRED_SERVER,
        )
        ...
        preference.summary = preference.entries
            ?.getOrNull(preference.findIndexOfValue(currentValue))
            ?: getString(R.string.settings_hdfull_preferred_server_summary)
        preference.setOnPreferenceChangeListener { pref, newValue ->
            val value = (newValue as String).trim()
            UserPreferences.setProviderCache(
                HdFullProvider,
    ...
    private fun setupParentalControlPreferences() {
        ...
        removePinPreference?.setOnPreferenceClickListener {
            changeParentalSettingWithPinCheck {
                ...
                ParentalControlNotifier.notifyChanged()
                updateParentalControlPreferenceState()
            }
            true
        }
    ...
    override fun onResume() {
        ...
        findPreference<MultiSelectListPreference>("SUBTITLE_LANGUAGES")?.values =
            UserPreferences.subtitleLanguages
    ...
    private fun showProfileAvatarDialogTv(profile: Profile) {
        ProfileAvatarPicker.show(requireContext(), viewLifecycleOwner, profile.avatarPath) { avatarPath ->
            lifecycleScope.launch {
                if (ProfileManager.setProfileAvatar(profile.id, avatarPath)) {
                    Toast.makeText(requireContext(), R.string.profile_avatar_changed, Toast.LENGTH_SHORT).show()
    ...
    private fun showProfileActionsDialogTv(profile: Profile, profileCount: Int = 1) {
        val items = mutableListOf<String>().apply {
            ...
            add(getString(R.string.profile_action_avatar))
            if (profileCount > 1) {
            ...
            .setItems(items.toTypedArray()) { _, which ->
                when (items[which]) {
                    getString(R.string.profile_action_switch) -> {
                        ...
                        val oldTheme = oldProfileId
                            ?.let { ProfileManager.getProfilePrefs(it).getString("SELECTED_THEME", ThemeManager.DEFAULT) }
                            ?: ThemeManager.DEFAULT
                        ProfileSwitchPinGuard.verifyCurrentProfile(requireContext()) {
                            viewLifecycleOwner.lifecycleScope.launch {
                                ProfileManager.switchToProfile(profile.id)
                            val newLang = AppLanguageManager.getProfileLanguage(requireContext(), profile.id)
                            val newTheme = ProfileManager.getProfilePrefs(profile.id)
                                .getString("SELECTED_THEME", ThemeManager.DEFAULT)
                                ?: ThemeManager.DEFAULT
                    ...
                    getString(R.string.profile_action_avatar) -> showProfileAvatarDialogTv(profile)
