...
class HomeMobileFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            // Home remains on the back stack when Profiles opens. Collecting
            // with the Fragment lifecycle can deliver a late result after the
            // view binding has been cleared.
        ...
        binding.ivProviderLogo.apply {
            ...
            setOnClickListener {
                findNavController().currentDestination?.id
                    ?.takeIf { it != R.id.providers }
                    ?.let { findNavController().navigate(R.id.providers) }
            }
        }
    ...
    private fun setupProfileHeader() {
        ...
        if (profile != null) {
            binding.ivProfileAvatar.loadProfileAvatar(profile.avatarPath)
            ...
            ?.also {
                it.list.forEach { show ->
                    when (show) {
                        ...
                        is LiveChannel -> show.itemType = AppAdapter.Type.LIVE_CHANNEL_SWIPER_MOBILE_ITEM
                    }
                }
            }
            ...
            ?.also {
                ...
                it.list.forEach { show ->
                    when (show) {
                        ...
                        is TvShow -> show.itemType = AppAdapter.Type.TV_SHOW_CONTINUE_WATCHING_MOBILE_ITEM
                        is LiveChannel -> show.itemType = AppAdapter.Type.LIVE_CHANNEL_MOBILE_ITEM
                    }
                }
            }
