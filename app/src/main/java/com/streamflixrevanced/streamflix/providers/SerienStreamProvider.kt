...
object SerienStreamProvider : Provider {
    ...
    fun initialize(context: Context) {
        ...
        if (webViewResolver == null) {
            webViewResolver = WebViewResolver(this.appContext)
        }
        if (tvShowDao == null) {
    ...
    private suspend fun inferTrashTvSeasonTitle(seasonId: String, fallback: String): String {
        val episodeTitle = runCatching {
        ...
        val withoutEpisodeSuffix = episodeTitle
            .replace(Regex("\\s+-\\s+Folge\\b.*$", RegexOption.IGNORE_CASE), "")
            .trim()
    ...
    override suspend fun getTvShow(id: String): TvShow {
        ...
        val localRating = if (tmdbTvShow?.rating == null) {
            ...
            val imdbDocument = if (imdbTitleUrl.isNotEmpty()) {
                runCatching { getService().getCustomUrl(imdbTitleUrl) }.getOrNull()
            } else {
                null
            }

            val imdbRating = imdbDocument
                ?.takeUnless { it.isAwsWafChallenge() }
                ?.selectFirst("div[data-testid='hero-rating-bar__aggregate-rating__score'] span")
                ?.text()
                ?.parseRatingValue()

            imdbRating ?: document.parseSerienStreamRating()
        } else {
            0.0
        }
        ...
        val seasonLinks = document.select("#season-nav ul li a").map {
            val seasonText = it.text().trim()
            val seasonNumber = seasonText.toIntOrNull() ?: 0
            Triple(
                getSeasonIdFromLink(it.attr("href")),
                seasonNumber,
                seasonText,
            )
        }
        val isTrashTv = title.startsWith("Trash-TV", ignoreCase = true)
        ...
        return TvShow(id = id,
            ...
            seasons = seasonLinks.map { (seasonId, seasonNumber, seasonText) ->
                val seasonTitle = if (isTrashTv && seasonNumber > 0) {
                    inferTrashTvSeasonTitle(seasonId, "Staffel $seasonNumber")
                } else {
                    if (seasonText == "Filme") "Filme" else "Staffel $seasonNumber"
                }
                Season(
                    id = seasonId,
                    number = seasonNumber,
    ...
    private fun Document.isAwsWafChallenge(): Boolean =
        selectFirst("script[src*='awswaf.com'], script[src*='challenge.js']") != null ||
            selectFirst("script:contains(window.gokuProps)") != null

    private fun String.parseRatingValue(): Double? {
        val value = trim().replace(',', '.').toDoubleOrNull() ?: return null
        return value.takeIf { it in 0.0..10.0 }
    }

    private fun Document.parseSerienStreamRating(): Double {
        val ratingText = selectFirst(".text-white-50:contains(Bewertungen), .text-white-50:contains(Bewertung)")
            ?.text()
            .orEmpty()

        // The provider displays review counts such as "16 Bewertungen".
        // A bare number is a count, not a rating. Only accept an explicit
        // score format or a rating label followed by the score.
        val score = Regex(
            """(?i)(?:bewertung(?:en)?\s*:?\s*)?(10(?:[,.]0)?|[0-9](?:[,.][0-9]+)?)\s*/\s*10"""
        ).find(ratingText)?.groupValues?.getOrNull(1)?.parseRatingValue()
            ?: Regex(
                """(?i)bewertung(?:en)?\s*:?\s*(10(?:[,.]0)?|[0-9](?:[,.][0-9]+)?)"""
            ).find(ratingText)?.groupValues?.getOrNull(1)?.parseRatingValue()
        ...
        for (element in elements) {
            ...
            try {
                ...
                val videoUrlString = runCatching {
                    ...
                    // If OkHttp cannot complete the redirect at all, use the
                    // episode page to establish Cloudflare clearance, then
                    // retry this exact redirect token.
     ...
     * challenged, OkHttp stops at s.to instead; let the WebView execute the
     * Turnstile flow and return the resulting iframe/player URL.
     */
    private suspend fun resolveUnfinishedRedirect(url: String, episodeUrl: String): String {
        ...
        return redirectResolverMutex.withLock {
            // A previous episode may already have established a valid
            // SerienStream session. Reuse it before opening another WebView
            // challenge; Turnstile tokens themselves are intentionally not
            // persisted because they are one-use and short-lived.
            ...
            val result = getWebViewResolver().getResult(
                // The /r?t=... URL is only a redirect token. The Cloudflare
                // challenge is attached to the episode page context, so load
                // that page first and keep its cookies in CookieManager.
                url = episodeUrl,
                headers = mapOf(
            ...
            // The WebView was used only to establish the episode-page
            // clearance. Retry the original token with the shared cookies.
    ...
    private fun extractTurnstileFields(html: String): TurnstileFields? {
        ...
        return TurnstileFields(csrfToken, turnstileToken, cloudflareResponse)
            .takeIf { it.csrfToken.isNotBlank() && it.turnstileToken.isNotBlank() && it.cloudflareResponse.isNotBlank() }
    }
