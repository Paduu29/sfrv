...
object RedflixProvider : Provider {
    ...
    override suspend fun getMovieFromProvider(id: String): Movie = service.movieDetails(id.toTmdbId()).toMovie(id.toTmdbId())

    override suspend fun getTvShowFromProvider(id: String): TvShow = service.tvDetails(id.toTmdbId()).toTvShow(id.toTmdbId())
    ...
    override suspend fun getServers(id: String, videoType: Video.Type): List<Video.Server> {
        val tmdbId = when (videoType) {
            is Video.Type.Movie -> videoType.id.toTmdbId()
            is Video.Type.Episode -> videoType.tvShow.id.toTmdbId()
        }

        val screenscapeUrl = "https://screenscape.me/embed".toHttpUrl().newBuilder()
            .addQueryParameter("tmdb", tmdbId)
            .addQueryParameter("type", if (videoType is Video.Type.Movie) "movie" else "tv")
            .apply {
                if (videoType is Video.Type.Episode) {
                    addQueryParameter("season", videoType.season.number.toString())
                    addQueryParameter("episode", videoType.number.toString())
                }
            }
            .build()

        val vidNestPath = when (videoType) {
            is Video.Type.Movie -> "movie/$tmdbId"
            is Video.Type.Episode ->
                "tv/$tmdbId/${videoType.season.number}/${videoType.number}"
        }

        return buildList {
            add(Video.Server(id = "Screenscape", name = "Screenscape", src = screenscapeUrl.toString()))
            addAll(VideasyExtractor().servers(videoType, language))
            add(Video.Server(id = "VidNest", name = "VidNest", src = "https://vidnest.fun/$vidNestPath"))
        }
    }
