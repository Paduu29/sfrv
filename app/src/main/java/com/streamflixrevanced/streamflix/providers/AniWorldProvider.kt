...
object AniWorldProvider : Provider {
    ...
    private var tvShowDatabase: AppDatabase? = null
    private var boundProfileId: String? = null
    private var scheduledProfileId: String? = null
    ...
    /** AniWorld uses a transparent placeholder in src and lazy-loads the real image. */
    ...
    private fun backgroundImageUrl(style: String?): String? {
        val path = Regex("""url\(\s*[\"']?([^\"')]+)""")
            .find(style.orEmpty())?.groupValues?.getOrNull(1)
    ...
    fun initialize(context: Context) {
        ...
        if (boundProfileId != profileId || tvShowDao == null) {
            preloadJob?.cancel()
            preloadJob = null
            ...
            tvShowDatabase = AppDatabase.getInstanceForProvider(name, appContext, profileId)
            tvShowDao = tvShowDatabase?.tvShowDao()
            boundProfileId = profileId
        }
        if (scheduledProfileId != profileId) {
        ...
        val workRequest = OneTimeWorkRequestBuilder<AniWorldUpdateTvShowWorker>()
            ...
            .setInputData(
                androidx.work.workDataOf(
                    AniWorldUpdateTvShowWorker.PROFILE_ID to (ProfileManager.activeProfileId ?: "default"),
                )
            )
            .build()

        WorkManager.getInstance(context).enqueueUniqueWork(
            "AniWorldUpdateTvShowWorker_${ProfileManager.activeProfileId ?: "default"}",
    ...
    private fun getDao(): TvShowDao {
        val activeProfileId = ProfileManager.activeProfileId ?: "default"
                    ...
                    .map {
                        TvShow(
                            ...
                            title = it.selectFirst("a h3")
                                ?.text()
                                ?: "",
                            poster = imageUrl(it.selectFirst("img")),
                        )
                    }
            )
        )
    ...
    override suspend fun getTvShow(id: String): TvShow {
        ...
        val tvShow = TvShow(
            ...
            trailer = document.selectFirst("div[itemprop='trailer'] a")
                ?.attr("href"),
            poster = imageUrl(document.selectFirst("div.seriesCoverBox img")),
            banner = backgroundImageUrl(
                document.selectFirst("#series > section > div.backdrop")?.attr("style")
            ),
