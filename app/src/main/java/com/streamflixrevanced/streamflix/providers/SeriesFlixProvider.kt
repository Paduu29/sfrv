...
object SeriesFlixProvider : Provider {
        ...
        document.select("section").forEach { section ->
            ...
            val items = when {
                ...
                title.contains("Ultimos Episodios", ignoreCase = true) ||
                    ...
                    section.select("article.TPost.B").mapNotNull(::parseEpisodeCardAsTvShow)
                else ->
    ...
    override suspend fun search(query: String, page: Int): List<AppAdapter.Item> {
        ...
        return document.select("article.TPost.B").mapNotNull { element ->
            parseShowCard(element) ?: parseEpisodeCardAsTvShow(element)
        }
    }
    ...
    override suspend fun getTvShowFromProvider(id: String): TvShow {
        ...
        val seasons = document.select("a[href*='/temporada/']").mapNotNull { seasonAnchor ->
            ...
            // Only add seasons with valid (non-zero) season numbers
            number?.takeIf { it > 0 }?.let { validNumber ->
                Season(
                    id = href,
                    number = validNumber,
                    title = "Temporada $validNumber",
                    poster = banner
                )
            }
    ...
    private fun parseEpisodeCardAsTvShow(element: Element): TvShow? {
        val anchor = element.selectFirst("a[href*='/episodio/']") ?: return null

        val episodeUrl = anchor.attr("href").trim()

        val showUrl = episodeUrl
            .replace("/episodio/", "/serie/")
            .replace(Regex("-\\d+x\\d+/?$"), "/")
