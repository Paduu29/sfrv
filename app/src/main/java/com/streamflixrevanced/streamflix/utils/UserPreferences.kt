...
object UserPreferences {
    ...
    var autoSkipIntro: Boolean
        get() = Key.AUTO_SKIP_INTRO.getBoolean() ?: false
        set(value) {
            Key.AUTO_SKIP_INTRO.setBoolean(value)
        }
    ...
    fun getFavoriteProviders(profileId: String): Set<String> =
        ProfileManager.getProfilePrefs(profileId)
            .getStringSet(Key.FAVORITE_PROVIDERS.name, emptySet())
            ?.toSet()
            .orEmpty()

    fun setFavoriteProviders(profileId: String, providers: Set<String>) {
        ProfileManager.getProfilePrefs(profileId)
            .edit()
            .putStringSet(Key.FAVORITE_PROVIDERS.name, providers.toSet())
            .apply()
    }
