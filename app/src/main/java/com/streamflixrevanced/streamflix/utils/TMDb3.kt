    ...
    private interface ApiService {
        ...
        companion object {
            fun build(): ApiService {
                // The build-time pool is shared by the release, while the
                // existing user preference contributes one additional key.
                // Keep user configuration separate so it can be appended
                // without replacing the release pool.
                val buildKeyPool = BuildConfig.TMDB_API_KEYS
                    .ifBlank { BuildConfig.TMDB_API_KEY }
                val configuredKeys = (buildKeyPool + "," + UserPreferences.tmdbApiKey)
                    .split(Regex("[,;\\s]+"))
                    .map(String::trim)
                    .filter(String::isNotEmpty)
                    .distinct()
                val apiKeys = (configuredKeys.ifEmpty { listOf(BuildConfig.TMDB_API_KEY) })
                    .filter(String::isNotEmpty)
                    .ifEmpty { listOf("") }
                val nextKeyIndex = AtomicInteger(0)

                val client = OkHttpClient.Builder().addInterceptor { chain ->
                    val original = chain.request()

                    fun requestFor(apiKey: String) = original.newBuilder()
                        .url(
                    ...
                    fun nextApiKey(): String =
                        apiKeys[nextKeyIndex.getAndIncrement() % apiKeys.size]

                    var response = chain.proceed(requestFor(nextApiKey()))
                    if (response.code == 401 || response.code == 429) {
                        for (attempt in 1 until apiKeys.size) {
                            if (response.code != 401 && response.code != 429) break
                            response.close()
                            response = chain.proceed(requestFor(nextApiKey()))
                        }
                    }
                    response
                }.build()
