...
object InAppUpdater {
    ...
    private const val GITHUB_OWNER = "PaduU29-SFRV"
    private const val GITHUB_REPO = "streamflix-revanced"

    internal data class Version(val name: String) : Comparable<Version> {
    ...
    suspend fun getReleaseUpdate(): GitHub.Release? {
        ...
        if (Version(normalizeVersion(latestRelease.tagName)) > Version(currentVersion)) {
    ...
    suspend fun getNewReleases(): List<GitHub.Release> {
        ...
        val newReleases = releases
            .filter { release ->
                isStablePublishedRelease(
                    draft = release.draft,
                    prerelease = release.prerelease,
                    publishedAt = release.publishedAt,
                ) &&
                    Version(normalizeVersion(release.tagName)) > Version(currentVersion)
            }
            .sortedByDescending { Version(normalizeVersion(it.tagName)) }

        return newReleases
    }

    internal fun normalizeVersion(tagName: String): String =
        tagName.trim().removePrefix("v").removePrefix("V")

    internal fun isStablePublishedRelease(
        draft: Boolean,
        prerelease: Boolean,
        publishedAt: String?,
    ): Boolean = !draft && !prerelease && publishedAt != null
    ...
    internal fun expectedAssetName(
        ...
        val suffix = when (appLayout) {
            "mobile" -> "-only-mobile"
            "tv" -> "-only-tv"
            else -> ""
        }
        return "$repository-${tagName.trim()}$suffix.apk"
    }

    internal fun selectAsset(
        release: GitHub.Release,
        appLayout: String,
    ): GitHub.Release.Asset? = release.assets.firstOrNull { asset ->
        asset.name == expectedAssetName(GITHUB_REPO, release.tagName, appLayout)
    }
