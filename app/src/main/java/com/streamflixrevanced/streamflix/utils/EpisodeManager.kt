...
object EpisodeManager {
    ...
    suspend fun addEpisodesFromDb(type: Video.Type.Episode, database: AppDatabase) {
        ...
        if (episodesFromDb.isNotEmpty()) {
            episodesFromDb.forEach { episode ->
                ...
                // Room stores the season relation as an ID. Its converter recreates
                // Season(id, 0), so resolve the actual season entity before using it.
                episode.season = episode.season?.id
                    ?.let { seasonId -> database.seasonDao().getById(seasonId) }
                    ?: seasonContext
            }
    ...
    suspend fun ensureNextEpisodeAvailable(type: Video.Type.Episode, database: AppDatabase): Boolean {
        ...
        if (nextSeasonEpisodes.isEmpty() && seasonToLoad.id.isNotBlank()) {
            nextSeasonEpisodes = runCatching {
                ...
                if (fetchedEpisodes.isNotEmpty()) {
                    database.seasonDao().insert(seasonToLoad)
                    fetchedEpisodes.forEach { episode ->
    ...
    fun updateTvShowImdbId(tvShowId: String, imdbId: String) {
        episodes.indices.forEach { index ->
            val episode = episodes[index]
            if (episode.tvShow.id == tvShowId && episode.tvShow.imdbId.isNullOrBlank()) {
                episodes[index] = episode.copy(
                    tvShow = episode.tvShow.copy(imdbId = imdbId)
                )
            }
        }
    }
        ...
        val videoEpisodes = episodes.map { ep ->
            ...
            Episode(
                ...
                season = Episode.Season(
                    number = seasonFromDb?.number?.takeIf { it > 0 } ?: seasonNumber,
