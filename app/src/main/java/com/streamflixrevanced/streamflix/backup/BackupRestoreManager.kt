...
class BackupRestoreManager(
    ...
    suspend fun importUserData(json: String): Boolean {
        return try {
            ...
            var importedProviderCount = 0
            var importedRecordCount = 0

            Log.d(
                TAG,
                "Starting import from version $backupVersion for ${providersArray.length()} providers " +
                    "into profile ${ProfileManager.activeProfileId}",
            )

            for (i in 0 until providersArray.length()) {
                val providerObj = providersArray.optJSONObject(i) ?: continue
                val providerName = providerObj.optString("name").takeIf { it.isNotBlank() } ?: continue
                val providerCtx = providers.firstOrNull { it.name == providerName }
                ...
                providerObj.optJSONArray("seasons")?.let { arr ->
                    ...
                    if (seasonsToSave.isNotEmpty()) {
                        ...
                        importedRecordCount += seasonsToSave.size
                ...
                providerObj.optJSONArray("tvShows")?.let { arr ->
                    for (j in 0 until arr.length()) {
                        ...
                        providerCtx.tvShowDao.restoreFromBackup(tvShow)
                        importedRecordCount++
                ...
                providerObj.optJSONArray("movies")?.let { arr ->
                    for (j in 0 until arr.length()) {
                        ...
                        providerCtx.movieDao.restoreFromBackup(movie)
                        importedRecordCount++
                ...
                providerObj.optJSONArray("episodes")?.let { arr ->
                    for (j in 0 until arr.length()) {
                        ...
                        providerCtx.episodeDao.restoreFromBackup(ep)
                        importedRecordCount++
            ...
            if (importedProviderCount == 0 || importedRecordCount == 0) {
                Log.w(
                    TAG,
                    "Import contained no restorable records: matchedProviders=$importedProviderCount, " +
                        "records=$importedRecordCount, targetProfile=${ProfileManager.activeProfileId}",
                )
                false
            } else {
                Log.d(
                    TAG,
                    "Import completed successfully: providers=$importedProviderCount, " +
                        "records=$importedRecordCount, targetProfile=${ProfileManager.activeProfileId}",
                )
                true
            }
