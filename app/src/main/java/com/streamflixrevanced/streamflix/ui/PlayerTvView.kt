...
class PlayerTvView @JvmOverloads constructor(
    ...
    var onOverlayPrimaryAction: (() -> Boolean)? = null

    private var zoomToast: Toast? = null
    private var consumeOverlayPrimaryActionKeyUp = false
    ...
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        ...
        if (
            ...
            if (event.action == KeyEvent.ACTION_UP && consumeOverlayPrimaryActionKeyUp) {
                consumeOverlayPrimaryActionKeyUp = false
                return true
            }
            if (event.action == KeyEvent.ACTION_DOWN) {
                val handled = onOverlayPrimaryAction?.invoke() == true
                consumeOverlayPrimaryActionKeyUp = handled
