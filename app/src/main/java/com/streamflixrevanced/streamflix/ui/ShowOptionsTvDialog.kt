...
class ShowOptionsTvDialog(
    ...
    init {
        ...
        when (show) {
            ...
            is LiveChannel -> displayTvShow(show.asTvShow())
        }
    ...
    private fun displayEpisode(episode: Episode) {
        binding.btnOptionShowDetails.visibility = View.GONE
        binding.btnOptionRemoveRecentlyWatched.visibility = View.GONE
        val provider = UserPreferences.currentProvider

        Glide.with(context)
            .load(episode.poster ?: episode.tvShow?.poster ?: R.drawable.glide_fallback_cover)
    ...
    private fun displayMovie(movie: Movie) {
        ...
        binding.btnOptionShowDetails.apply {
            text = context.getString(R.string.option_show_details_movie)
            setOnClickListener {
                when (val fragment = context.toActivity()?.getCurrentFragment()) {
                    is HomeTvFragment -> androidx.navigation.fragment.NavHostFragment.findNavController(fragment).navigate(
                        HomeTvFragmentDirections.actionHomeToMovie(id = movie.id)
                    )
                    is HomeMobileFragment -> androidx.navigation.fragment.NavHostFragment.findNavController(fragment).navigate(
            ...
            visibility = when (context.toActivity()?.getCurrentFragment()) {
                is HomeTvFragment, is HomeMobileFragment -> View.VISIBLE
                else -> View.GONE
            }
        }
        ...
        binding.btnOptionRemoveRecentlyWatched.apply {
            setOnClickListener {
                checkProviderAndRun(freshMovie) {
                    val provider = UserPreferences.currentProvider ?: return@checkProviderAndRun
                    context.toActivity()?.lifecycleScope?.launch(Dispatchers.IO) {
                        database.movieDao().removeFromRecentlyWatched(freshMovie.id)
                        val updatedMovie = database.movieDao().getById(freshMovie.id) ?: freshMovie
                        UserDataCache.syncMovieToCache(context, provider, updatedMovie)
                    }
                }
                hide()
            }
            visibility = if (freshMovie.lastPlayedAtMillis != null) View.VISIBLE else View.GONE
        }
    }
    ...
    private fun displayTvShow(tvShow: TvShow) {
        ...
        binding.btnOptionShowDetails.apply {
            text = context.getString(R.string.option_show_details_tv_show)
            setOnClickListener {
                when (val fragment = context.toActivity()?.getCurrentFragment()) {
                    is HomeTvFragment -> androidx.navigation.fragment.NavHostFragment.findNavController(fragment).navigate(
                        HomeTvFragmentDirections.actionHomeToTvShow(
        ...
        binding.btnOptionRemoveRecentlyWatched.apply {
            setOnClickListener {
                checkProviderAndRun(freshTvShow) {
                    val provider = UserPreferences.currentProvider ?: return@checkProviderAndRun
                    context.toActivity()?.lifecycleScope?.launch(Dispatchers.IO) {
                        database.tvShowDao().removeFromRecentlyWatched(freshTvShow.id)
                        val updatedTvShow = database.tvShowDao().getById(freshTvShow.id) ?: freshTvShow
                        UserDataCache.syncTvShowToCache(context, provider, updatedTvShow)
                    }
                }
                hide()
            }
            visibility = if (freshTvShow.lastPlayedAtMillis != null) View.VISIBLE else View.GONE
        }
    }

    private fun LiveChannel.asTvShow() = TvShow(
        id = id,
        title = name,
        overview = overview,
        poster = logo,
        ...
        favoritedAtMillis = this@asTvShow.favoritedAtMillis
        currentProgram = this@asTvShow.currentProgram?.title
    }
}
